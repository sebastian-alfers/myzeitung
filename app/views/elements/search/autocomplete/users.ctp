	...
	<?php foreach($user_documents as $user): ?>
		<?php if($user instanceof Apache_Solr_Document): ?>
			<li class="user art autoresult">
				...
				$link_data['additional'] = 'display:inline-block;';
				...
				<?php if(isset($user->user_name) && !empty($user->user_name)): ?>
					...
					<p><?php echo $this->Html->link( $this->MzText->truncate($user->user_name, $maxNameLen,array('ending' => '...', 'exact' => true, 'html' => false)), array('controller' => 'users', 'action' => 'view', 'username' => strtolower($user->user_username))) ?></p>
