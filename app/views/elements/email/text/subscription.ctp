...
<?php $subscriberLink = $this->Html->url(array('controller' => 'users', 'action' => 'view', 'username' => strtolower($paper['User']['username'])),true);
      $subscriberName = $this->MzText->generateDisplayName($paper['User'], false);
...
<?php echo sprintf(__('User %1$s subscribed to all of your posts into the paper %2$s',true),$subscriberName,$paper['Paper']['title'])."\n";?>
...
echo sprintf(__('User %1$s subscribed to the posts of your topic %2$s into the paper %3$s',true),$subscriberName,$topic['Topic']['name'],$paper['Paper']['title'])."\n";
...
if($topic == null &&  $category != null){
echo sprintf(__('User %1$s subscribed to all of your posts into the category %2$s of the paper %3$s',true),$subscriberName,$category['Category']['name'],$paper['Paper']['title'])."\n";
echo sprintf(__('Have a look at the profile of your subscriber: %s', true),$subscriberLink)."\n";
echo sprintf(__('Have a look at the paper category you have been subscribed to: %s', true),$categoryLink)."\n";
}
if($topic != null &&  $category != null){
echo sprintf(__('User %1$s subscribed to the posts of your topic %2$s into the category %3$s of the paper %4$s',true),$subscriberName,$topic['Topic']['name'],$category['Category']['name'],$paper['Paper']['title'])."\n";
echo sprintf(__('Have a look at the profile of your subscriber: %s', true),$subscriberLink)."\n";
echo sprintf(__('Have a look at the paper category you have been subscribed to: %s', true),$categoryLink)."\n";
}
?>
