...
<ul>
      ...
      <li><?php echo $this->Html->link(sprintf(__n('%s paper subscription', '%s paper subscriptions', $user['User']['subscription_count'],true), $this->MzNumber->format($user['User']['subscription_count'],'.')), array('controller' => 'users', 'action' => 'viewSubscriptions', 'username' =>strtolower($session->read('Auth.User.username')),'own_paper' => Paper::FILTER_SUBSCRIBED), array('rel' => 'nofollow'));?></li>
      <li><?php echo $this->Html->link(sprintf(__n('%s paper', '%s papers', $user['User']['paper_count'],true), $this->MzNumber->format($user['User']['paper_count'],'.')), array('controller' => 'users', 'action' => 'viewSubscriptions', 'username' =>strtolower($session->read('Auth.User.username')),'own_paper' => Paper::FILTER_OWN), array('rel' => 'nofollow'));?></li>
  </ul>
