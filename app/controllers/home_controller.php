...
class HomeController extends AppController {
	...
	public function index(){
		...
		$paper_options = array('conditions' => array('enabled' => true), 'limit' => 9, 'order' => array('created DESC'), 'fields' => array('id', 'image', 'title', 'description'));
		$this->set('papers', $this->Paper->find('all', $paper_options));
		//loading users
		$this->User->contain();
		$user_options = array('conditions' => array('enabled' => true),'limit' => 12, 'order' => array('created DESC'), 'fields' => array('id', 'image', 'username', 'name', 'description'));
		$this->set('users', $this->User->find('all', $user_options));
		//loading users
		$this->Post->contain('Route', 'User.id', 'User.username', 'User.name', 'User.image');
		$post_options = array('conditions' => array('enabled' => true),'limit' => 7, 'order' => array('Post.created DESC'), 'fields' => array('id','title'));
