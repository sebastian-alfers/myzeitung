...
if(name.numeric)
return Entities.encodeNumeric;return Entities.encodeRaw;},decode:function(text){return text.replace(entityRegExp,function(all,numeric,value){if(numeric){value=parseInt(value);if(value>0xFFFF){value-=0x10000;return String.fromCharCode(0xD800+(value>>10),0xDC00+(value&0x3FF));}else
return asciiMap[value]||String.fromCharCode(value);}
return reverseEntities[all]||namedEntities[all]||nativeDecode(all);});}};})(tinymce);tinymce.html.Styles=function(settings,schema){var rgbRegExp=/rgb\s*\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*\)/gi,urlOrStrRegExp=/(?:url(?:(?:\(\s*\"([^\"]+)\"\s*\))|(?:\(\s*\'([^\']+)\'\s*\))|(?:\(\s*([^)\s]+)\s*\))))|(?:\'([^\']+)\')|(?:\"([^\"]+)\")/gi,styleRegExp=/\s*([^:]+):\s*([^;]+);?/g,trimRightRegExp=/\s+$/,urlColorRegExp=/rgb/,undef,i,encodingLookup={},encodingItems;settings=settings||{};encodingItems='\\" \\\' \\; \\: ; : _'.split(' ');for(i=0;i<encodingItems.length;i++){encodingLookup[encodingItems[i]]='_'+i;encodingLookup['_'+i]=encodingItems[i];}
...
return;styles[target]=styles[a]+' '+styles[b]+' '+styles[c];delete styles[a];delete styles[b];delete styles[c];};function encode(str){isEncoded=true;return encodingLookup[str];};function decode(str,keep_slashes){if(isEncoded){str=str.replace(/_[0-9]/g,function(str){return encodingLookup[str];});}
if(!keep_slashes)
str=str.replace(/\\([\'\";:])/g,"$1");return str;}
if(css){css=css.replace(/\\[\"\';:_]/g,encode).replace(/\"[^\"]+\"|\'[^\']+\'/g,function(str){return str.replace(/[;:]/g,encode);});while(matches=styleRegExp.exec(css)){name=matches[1].replace(trimRightRegExp,'').toLowerCase();value=matches[2].replace(trimRightRegExp,'');if(name&&value.length>0){if(name==='font-weight'&&value==='700')
...
return styles;},serialize:function(styles,element_name){var css='',name,value;function serializeStyles(name){var styleList,i,l,name,value;styleList=schema.styles[name];if(styleList){for(i=0,l=styleList.length;i<l;i++){name=styleList[i];value=styles[name];if(value!==undef&&value.length>0)
css+=(css.length>0?' ':'')+name+': '+value+';';}}};if(element_name&&schema&&schema.styles){serializeStyles('*');serializeStyles(name);}else{for(name in styles){value=styles[name];if(value!==undef&&value.length>0)
css+=(css.length>0?' ':'')+name+': '+value+';';}}
return css;}};};(function(tinymce){var transitional={},boolAttrMap,blockElementsMap,shortEndedElementsMap,nonEmptyElementsMap,whiteSpaceElementsMap,selfClosingElementsMap,makeMap=tinymce.makeMap,each=tinymce.each;function split(str,delim){return str.split(delim||',');};function unpack(lookup,data){var key,elements={};function replace(value){return value.replace(/[A-Z]+/g,function(key){return replace(lookup[key]);});};for(key in lookup){if(lookup.hasOwnProperty(key))
...
return true;}});tinymce.extend(Node,{create:function(name,attrs){var node,attrName;node=new Node(name,typeLookup[name]||1);if(attrs){for(attrName in attrs)
...
node=node.parent;}}},schema);rootNode=node=new Node(settings.root_name,11);parser.parse(html);if(validate)
fixInvalidChildren(invalidChildren);for(name in matchedNodes){list=nodeFilters[name];nodes=matchedNodes[name];fi=nodes.length;while(fi--){if(!nodes[fi].parent)
nodes.splice(fi,1);}
...
if(isIE){('abbr article aside audio canvas '+'details figcaption figure footer '+'header hgroup mark menu meter nav '+'output progress section summary '+'time video').replace(/\w+/g,function(name){d.createElement(name);});}
...
return false;attributes=self.getAttribs(node);i=node.attributes.length;while(i--){name=node.attributes[i].nodeName;if(name==="name"||name.indexOf('data-')===0)
return false;}}
...
tinymce.removeUnload(t.destroy);},createRng:function(){var d=this.doc;return d.createRange?d.createRange():new tinymce.dom.Range(this);},nodeIndex:function(node,normalized){var idx=0,lastNodeType,lastNode,nodeType,nodeValueExists;if(node){for(lastNodeType=node.nodeType,node=node.previousSibling,lastNode=node;node;node=node.previousSibling){nodeType=node.nodeType;if(normalized&&nodeType==3){nodeValueExists=false;try{nodeValueExists=node.nodeValue.length}catch(c){}
...
return domRange;collapsed=selection.isCollapsed();if(ieRange.item||!element.hasChildNodes()){if(collapsed){domRange.setStart(element,0);domRange.setEnd(element,0);}else{domRange.setStart(element.parentNode,dom.nodeIndex(element));domRange.setEnd(domRange.startContainer,domRange.startOffset+1);}
return domRange;}
function findEndPoint(start){var marker,container,offset,nodes,startIndex=0,endIndex,index,parent,checkRng,position;checkRng=ieRange.duplicate();checkRng.collapse(start);marker=dom.create('a');parent=checkRng.parentElement();if(!parent.hasChildNodes()){domRange[start?'setStart':'setEnd'](parent,0);return;}
...
setEndPoint(true);setEndPoint();ieRng.select();};this.getRangeAt=function(){if(!range||!tinymce.dom.RangeUtils.compareRanges(lastIERng,selection.getRng())){range=getRange();lastIERng=selection.getRng();}
...
caretNode=self.dom.get('__caret');rng=doc.createRange();rng.setStartBefore(caretNode);rng.setEndBefore(caretNode);self.setRng(rng);self.dom.remove('__caret');self.setRng(rng);}else{if(rng.item){doc.execCommand('Delete',false,null);rng=self.getRng();}
...
t.tridentSel.destroy();if(bookmark){if(bookmark.start){rng=dom.createRng();root=dom.getRoot();function setEndPoint(start){var point=bookmark[start?'start':'end'],i,node,offset,children;if(point){offset=point[0];for(node=root,i=point.length-1;i>=1;i--){children=node.childNodes;if(point[i]>children.length-1)
...
return true;};if(setEndPoint(true)&&setEndPoint()){t.setRng(rng);}}else if(bookmark.id){function restoreEndPoint(suffix){var marker=dom.get(bookmark.id+'_'+suffix),node,idx,next,prev,keep=bookmark.keep;if(marker){node=marker.parentNode;if(suffix=='start'){if(!keep){idx=dom.nodeIndex(marker);}else{node=marker.firstChild;idx=1;}
...
t.init();});};loadScripts();},init:function(){var n,t=this,s=t.settings,w,h,e=t.getElement(),o,ti,u,bi,bc,re,i;tinymce.add(t);s.aria_label=s.aria_label||DOM.getAttrib(e,'aria-label',t.getLang('aria.rich_text_area'));if(s.theme){s.theme=s.theme.replace(/-/,'');o=ThemeManager.get(s.theme);t.theme=new o();if(t.theme.init&&s.init_theme)
...
n=DOM.add(o.iframeContainer,'iframe',{id:t.id+"_ifr",src:u||'javascript:""',frameBorder:'0',title:s.aria_label,style:{width:'100%',height:h}});t.contentAreaContainer=o.iframeContainer;DOM.get(o.editorContainer).style.display=t.orgDisplay;DOM.get(t.id).style.display='none';DOM.setAttrib(t.id,'aria-hidden',true);if(!tinymce.relaxedDomain||!u)
...
t.schema=new tinymce.html.Schema(s);t.dom=new tinymce.dom.DOMUtils(t.getDoc(),{keep_values:true,url_converter:t.convertURL,url_converter_scope:t,hex_colors:s.force_hex_style_colors,class_filter:s.class_filter,update_styles:1,fix_ie_paragraphs:1,schema:t.schema});t.parser=new tinymce.html.DomParser(s,t.schema);t.parser.addAttributeFilter('name',function(nodes,name){var i=nodes.length,sibling,prevSibling,parent,node;while(i--){node=nodes[i];if(node.name==='a'&&node.firstChild){parent=node.parent;sibling=node.lastChild;do{prevSibling=sibling.prev;parent.insert(sibling,node);sibling=prevSibling;}while(sibling);}}});t.parser.addAttributeFilter('src,href,style',function(nodes,name){var i=nodes.length,node,dom=t.dom,value;while(i--){node=nodes[i];value=node.attr(name);if(name==="style")
node.attr('data-mce-style',dom.serializeStyle(dom.parseStyle(value),node.name));else
node.attr('data-mce-'+name,t.convertURL(value,name,node.name));}});t.parser.addNodeFilter('script',function(nodes,name){var i=nodes.length;while(i--)
...
e.value=h;o.element=e=null;return h;},setContent:function(content,args){var self=this,rootNode,body=self.getBody();args=args||{};args.format=args.format||'html';args.set=true;args.content=content;if(!args.no_events)
self.onBeforeSetContent.dispatch(self,args);content=args.content;if(!tinymce.isIE&&(content.length===0||/^\s+$/.test(content))){body.innerHTML='<br data-mce-bogus="1" />';return;}
...
self.onSetContent.dispatch(self,args);return args.content;},getContent:function(args){var self=this,content;args=args||{};args.format=args.format||'html';args.get=true;if(!args.no_events)
...
re=t.resizeInfo={node:e,ev:ev,cb:cb};});t.onKeyDown.add(function(ed,e){var sel;switch(e.keyCode){case 8:sel=t.getDoc().selection;if(sel.createRange&&sel.createRange().item){ed.dom.remove(sel.createRange().item(0));return Event.cancel(e);}}});}
...
addUndo();});t.dom.bind(t.dom.getRoot(),'dragend',function(e){addUndo();});t.onKeyUp.add(function(ed,e){var rng,parent,bookmark;if(isIE&&e.keyCode==8){rng=t.selection.getRng();if(rng.parentElement){parent=rng.parentElement();bookmark=t.selection.getBookmark();parent.innerHTML=parent.innerHTML;t.selection.moveToBookmark(bookmark);}}
if((e.keyCode>=33&&e.keyCode<=36)||(e.keyCode>=37&&e.keyCode<=40)||e.keyCode==13||e.keyCode==45||e.ctrlKey)
addUndo();});t.onKeyDown.add(function(ed,e){var rng,parent,bookmark,keyCode=e.keyCode;if(isIE&&keyCode==46){rng=t.selection.getRng();if(rng.parentElement){parent=rng.parentElement();if(!t.undoManager.typing){t.undoManager.beforeChange();t.undoManager.typing=true;t.undoManager.add();}
if(e.ctrlKey){rng.moveEnd('word',1);rng.select();}
...
addUndo();return;}
if((keyCode<16||keyCode>20)&&keyCode!=224&&keyCode!=91&&!t.undoManager.typing){t.undoManager.beforeChange();t.undoManager.add();t.undoManager.typing=true;}});t.onMouseDown.add(function(){if(t.undoManager.typing)
addUndo();});}
...
toggleFormat(command,value);},RemoveFormat:function(command){editor.formatter.remove(command);},mceBlockQuote:function(command){toggleFormat('blockquote');},FormatBlock:function(command,ui,value){return toggleFormat(value||'p');},mceCleanup:function(){var bookmark=selection.getBookmark();editor.setContent(editor.getContent({cleanup:TRUE}),{cleanup:TRUE});selection.moveToBookmark(bookmark);},mceRemoveNode:function(command,ui,value){var node=value||selection.getNode();if(node!=editor.getBody()){storeSelection();editor.dom.remove(node,TRUE);restoreSelection();}},mceSelectNodeDepth:function(command,ui,value){var counter=0;dom.getParent(selection.getNode(),function(node){if(node.nodeType==1&&counter++==value){selection.select(node);return FALSE;}},editor.getBody());},mceSelectNode:function(command,ui,value){selection.select(value);},mceInsertContent:function(command,ui,value){var caretNode,rng,rootNode,parent,node,rng,nodeRect,viewPortRect,args;function findSuitableCaretNode(node,root_node,next){var walker=new tinymce.dom.TreeWalker(next?node.nextSibling:node.previousSibling,root_node);while((node=walker.current())){if((node.nodeType==3&&tinymce.trim(node.nodeValue).length)||node.nodeName=='BR'||node.nodeName=='IMG')
return node;if(next)
walker.next();else
walker.prev();}};args={content:value,format:'html'};selection.onBeforeSetContent.dispatch(selection,args);value=args.content;if(value.indexOf('{$caret}')==-1)
value+='{$caret}';selection.setContent('<span id="__mce">\uFEFF</span>',{no_events:false});dom.setOuterHTML('__mce',value.replace(/\{\$caret\}/,'<span data-mce-type="bookmark" id="__mce">\uFEFF</span>'));caretNode=dom.select('#__mce')[0];rootNode=dom.getRoot();if(caretNode.previousSibling&&dom.isBlock(caretNode.previousSibling)||caretNode.parentNode==rootNode){node=findSuitableCaretNode(caretNode,rootNode);if(node){if(node.nodeName=='BR')
node.parentNode.insertBefore(caretNode,node);else
dom.insertAfter(caretNode,node);}}
while(caretNode){if(caretNode===rootNode){dom.setOuterHTML(parent,new tinymce.html.Serializer({},editor.schema).serialize(editor.parser.parse(dom.getOuterHTML(parent))));break;}
parent=caretNode;caretNode=caretNode.parentNode;}
caretNode=dom.select('#__mce')[0];if(caretNode){node=findSuitableCaretNode(caretNode,rootNode)||findSuitableCaretNode(caretNode,rootNode,true);dom.remove(caretNode);if(node){rng=dom.createRng();if(node.nodeType==3){rng.setStart(node,node.length);rng.setEnd(node,node.length);}else{if(node.nodeName=='BR'){rng.setStartBefore(node);rng.setEndBefore(node);}else{rng.setStartAfter(node);rng.setEndAfter(node);}}
selection.setRng(rng);if(!tinymce.isIE){node=dom.create('span',null,'\u00a0');rng.insertNode(node);nodeRect=dom.getRect(node);viewPortRect=dom.getViewPort(editor.getWin());if((nodeRect.y>viewPortRect.y+viewPortRect.h||nodeRect.y<viewPortRect.y)||(nodeRect.x>viewPortRect.x+viewPortRect.w||nodeRect.x<viewPortRect.x)){editor.getBody().scrollLeft=nodeRect.x;editor.getBody().scrollTop=nodeRect.y;}
dom.remove(node);}
...
value={href:value};value.href=value.href.replace(' ','%20');if(!link){if(tinymce.isWebKit){img=dom.getParent(selection.getNode(),'img');if(img){floatVal=img.style.cssFloat;img.style.cssFloat=null;}}
execNativeCommand('CreateLink',FALSE,'javascript:mctmp(0);');if(floatVal)
img.style.cssFloat=floatVal;each(dom.select("a[href='javascript:mctmp(0);']"),function(link){dom.setAttribs(link,value);});}else{if(value.href)
...
return value;}},'value');if(settings.custom_undo_redo){addCommands({Undo:function(){editor.undoManager.undo();},Redo:function(){editor.undoManager.redo();}});}};})(tinymce);(function(tinymce){var Dispatcher=tinymce.util.Dispatcher;tinymce.UndoManager=function(editor){var self,index=0,data=[];function getContent(){return tinymce.trim(editor.getContent({format:'raw',no_events:1}));};return self={typing:false,onAdd:new Dispatcher(self),onUndo:new Dispatcher(self),onRedo:new Dispatcher(self),beforeChange:function(){if(data[index])
data[index].beforeBookmark=editor.selection.getBookmark(2,true);},add:function(level){var i,settings=editor.settings,lastLevel;level=level||{};level.content=getContent();lastLevel=data[index];if(lastLevel&&lastLevel.content==level.content)
return null;if(settings.custom_undo_redo_levels){if(data.length>settings.custom_undo_redo_levels){for(i=0;i<data.length-1;i++)
...
return c;if(t==1&&c==s)
return n;}
return-1;},forceRoots:function(ed,e){var t=this,ed=t.editor,b=ed.getBody(),d=ed.getDoc(),se=ed.selection,s=se.getSel(),r=se.getRng(),si=-2,ei,so,eo,tr,c=-0xFFFFFF;var nx,bl,bp,sp,le,nl=b.childNodes,i,n,eid;for(i=nl.length-1;i>=0;i--){nx=nl[i];if(nx.nodeType===1&&nx.getAttribute('data-mce-type')){bl=null;continue;}
...
n.removeAttribute('id');r=d.createRange();r.setStartBefore(n);r.setEndBefore(n);se.setRng(r);}},getParentBlock:function(n){var d=this.dom;return d.getParent(n,d.isBlock);},insertPara:function(e){var t=this,ed=t.editor,dom=ed.dom,d=ed.getDoc(),se=ed.settings,s=ed.selection.getSel(),r=s.getRangeAt(0),b=d.body;var rb,ra,dir,sn,so,en,eo,sb,eb,bn,bef,aft,sc,ec,n,vp=dom.getViewPort(ed.getWin()),y,ch,car;ed.undoManager.beforeChange();rb=d.createRange();rb.setStart(s.anchorNode,s.anchorOffset);rb.collapse(TRUE);ra=d.createRange();ra.setStart(s.focusNode,s.focusOffset);ra.collapse(TRUE);dir=rb.compareBoundaryPoints(rb.START_TO_END,ra)<0;sn=dir?s.anchorNode:s.focusNode;so=dir?s.anchorOffset:s.focusOffset;en=dir?s.focusNode:s.anchorNode;eo=dir?s.focusOffset:s.anchorOffset;if(sn===en&&/^(TD|TH)$/.test(sn.nodeName)){if(sn.firstChild.nodeName=='BR')
...
aft.innerHTML=aft.firstChild.innerHTML;if(dom.isEmpty(bef))
...
e.innerHTML=isOpera?'\u00a0':'<br />';};if(dom.isEmpty(aft))
...
aft.normalize();bef.normalize();function first(n){return d.createTreeWalker(n,NodeFilter.SHOW_TEXT,null,FALSE).nextNode()||n;};r=d.createRange();r.selectNodeContents(isGecko?first(car||aft):car||aft);r.collapse(1);s.removeAllRanges();s.addRange(r);y=ed.dom.getPos(aft).y;if(y<vp.y||y+25>vp.y+vp.h){ed.getWin().scrollTo(0,y<vp.y?y:y-vp.h+25);}
...
s=extend({title:s.title,'class':'mce_'+id,unavailable_prefix:ed.getLang('unavailable',''),scope:s.scope,control_manager:t},s);id=t.prefix+id;if(s.menu_button){cls=cc||t._cls.menubutton||tinymce.ui.MenuButton;c=new cls(id,s,ed);ed.onMouseDown.add(c.hideMenu,c);}else{cls=t._cls.button||tinymce.ui.Button;c=new cls(id,s);}
...
currentWrapElm.appendChild(node);}else{currentWrapElm=0;each(tinymce.grep(node.childNodes),process);currentWrapElm=0;}};each(nodes,process);});if(format.wrap_links===false){each(newWrappers,function(node){function process(node){var i,currentWrapElm,children;if(node.nodeName==='A'){currentWrapElm=wrapElm.cloneNode(FALSE);newWrappers.push(currentWrapElm);children=tinymce.grep(node.childNodes);for(i=0;i<children.length;i++)
...
/* post/add_edit.js (53%) */
$(document).ready(function(){var is_paste=false;$("#dialog:ui-dialog").dialog("destroy");$("#dialog-topic").dialog({resizable:false,height:240,width:400,draggable:false,modal:true,autoOpen:false,buttons:{"Add and select":function(){saveTopic($('#new_topic').val());},Cancel:function(){$(this).dialog("close");}}});$("#dialog-url").dialog({resizable:false,height:240,width:400,draggable:false,modal:true,autoOpen:false,buttons:{"Add new URL":function(){prcoessUrl($('#url').val());},Cancel:function(){$(this).dialog("close");}}});$("#dialog-video-url").dialog({resizable:false,height:160,width:550,draggable:false,modal:true,autoOpen:false,buttons:{"Add new Video":function(){if(!is_paste){parseAndAddVideo($('#video-url').val());}},Cancel:function(){$(this).dialog("close");}}});$("#video-url").live('paste',function(e){is_pase=true;setTimeout(function(){parseAndAddVideo($('#video-url').val());},100);});$('.remove_li_item').live('click',function(){var img_path=$(this).attr('name');var post_id=$(this).attr('id');var req=$.post(base_url+'/posts/ajxRemoveImage/',{id:post_id,path:img_path}).success(function(string){}).error(function(){alert('error');});$(this).closest('li').remove();});$('#user_sidebar_content').show();$('#add_topick_link').bind('click',function(){topicDialog();});$('#btn-add-link').bind('click',function(){urlDialog();});$('#add_url_video_link').bind('click',function(){$('#video-url').val('');videoDialog();});function removeUrl(){alert('remove');}});function prcoessUrl(url){var max=30;var length=url.length;var text=url;if(length>max){text=url.substring(0,28);text+='...';}
if(url.substring(0,8)!='https://'&&url.substring(0,7)!='http://'){url='http://'+url;}
if(!isValidUrl(url)){return false;}
$('#links').append('<li id="'+url+'"><a href="'+url+'" title="'+url+'" target="blank">'+text+'</a><a class="remove_li_item"> - remove</a></li>');$('#url').val('');$("#dialog-url").dialog("close");}
function parseAndAddVideo(url){var hash=$('#hash').val();if(url=='')url='empty';var req=$.post(base_url+'/ajax/getVideoPreview.json',{url:url,hash:hash}).success(function(response){if(response.status=='failure'){if(response.data.msg!=''){alert(response.data.msg);}}
else if(response.status=='valid_url_no_video'){prcoessUrl(url);$("#dialog-video-url").dialog("close");}
else{addVideoItem(response.data.video_item_html);}
is_paste=false;}).error(function(){alert('request error');});}
function addVideoItem(video_item_html){$('#sortable').append(video_item_html);$("#dialog-video-url").dialog("close");scrollTo('sortable');}
function loadContentForUrl(url){alert('get content for '+url);var req=$.post(base_url+'/posts/url_content_extract/',{url:url}).success(function(string){alert(string);$('#url_content').html(string);}).error(function(){alert('error');});}
function saveTopic(val){var req=$.post(base_url+'/topics/ajax_add.json',{topic_name:val}).success(function(response){if(response.status=='success'){var new_topic_id=response.data;$('#SelectPostTopicId').append($('<option></option>').val(new_topic_id).html(val));$('#dialog-topic').dialog('close');$('#SelectPostTopicId').val(new_topic_id).text();}
else{alert('error while adding topic');}}).error(function(){alert('error');});}
function preSubmitActions(form){var len=$('#tmp_upload_stuff').children().length;if(len>0){alert('wait');$('.create-actions').prepend('<li>loading</li>');waitForSubmit(form);}
else{var links='';$('#links').children().each(function(index,element){links+=$(element).attr('id')+',';});$('#PostLinks').val(links);getTopicId();getAllowCommentsSetting();$('#PostImages').val($('#sortable').sortable('toArray'));var media_data=new Array();var j=0;$('#sortable li').each(function(index,element){if($(this).find('.item_data').children().length>0){var item_data=new Array();var i=0;$(this).find('.item_data').children().each(function(index,attribute){var tmp=new Array();tmp[0]=$(this).attr('name');tmp[1]=$(this).attr('value');item_data[i]=tmp;i++;});media_data[j]=item_data;j++;}});$('#PostMedia').val(JSON.stringify(media_data));$(form).submit();}}
function waitForSubmit(form){var len=$('#tmp_upload_stuff').children().length;if(len==0){prewaitForSubmits(form);}else{setTimeout(function(){waitForSubmit(form);},100);}}
function getTopicId(){$('#PostTopicId').val($('#SelectPostTopicId').val());return true;}
function getAllowCommentsSetting(){$('#PostAllowComments').val($('#SelectPostAllowComments').val());return true;}
function topicDialog(){$('#dialog-topic').dialog('open');return false;}
function urlDialog(){$('#dialog-url').dialog('open');return false;}
function videoDialog(){$('#dialog-video-url').dialog('open');return false;}
$(document).ready(function(){$('#PostImage').hide();$(function(){$('#file_upload').fileUploadUI({uploadTable:$('#sortable'),downloadTable:$('#sortable'),buildUploadRow:function(files,index){return $('<li id="tmp_upload_stuff"><table><tr><td class="file_upload_preview"><img style="position:relative; top:0px; left:10px;height:16px;width:16px;" src="'+base_url+'/img/assets/loader.gif" /><\/td>'+'<\/tr></table></li>');},buildDownloadRow:function(file){return $('<li class="ui-state-default teaser-sort"><a class="remove_li_item" name="'+file.path+'" style="cursor:pointer;vertical-align:top;">remove</a><img src="/'+file.path+'" width="100" \/><div class="item_data" style="display: none;"><input type="hidden" name="item_type" value="image" /><input type="hidden" name="img_name" value="'+file.name+'"></div></li>');},beforeSend:function(event,files,index,xhr,handler,callBack){if(files[index].size===0){handler.uploadRow.find('.file_upload_progress').html('FILE IS EMPTY!');setTimeout(function(){handler.removeNode(handler.uploadRow);},10000);return;}
var regexp=/\.(png)|(jpg)|(jpeg)|(gif)$/i;if(!regexp.test(files[index].name)){handler.uploadRow.find('.file_upload_progress').html('ONLY IMAGES ALLOWED!');setTimeout(function(){handler.removeNode(handler.uploadRow);},10000);return;}
if(files[index].size>1000000){handler.uploadRow.find('.file_upload_progress').html('FILE TOO BIG!');setTimeout(function(){handler.removeNode(handler.uploadRow);},10000);return;}
if(index===0){files.uploadSequence=[];files.uploadSequence.start=function(index){var next=this[index];if(next){next.apply(null,Array.prototype.slice.call(arguments,1));this[index]=null;}};}
files.uploadSequence.push(callBack);if(index+1===files.length){files.uploadSequence.start(0);}},onComplete:function(event,files,index,xhr,handler){files.uploadSequence.start(index+1);},onAbort:function(event,files,index,xhr,handler){handler.removeNode(handler.uploadRow);files.uploadSequence[index]=null;files.uploadSequence.start(index+1);}});});});